...
const POOL_BY_CHAIN_AND_PROTOCOL = {
  sepolia: {
    ...
    // Liquity: this is BorrowerOperations (ETH branch), which is what
    // users call and what the on-chain contract validates tx.to against —
    // NOT TroveManager, which only emits events. See liquityDecoder.ts.
