/**
 * supply-to-compound.js
 *
 * Script to perform a supply transaction to Compound Comet (USDC market) on Sepolia.
 ...
 *
 * Usage: node scripts/supply-to-compound.js
 */
...
async function main() {
  ...
  // Supply amount (will be adjusted based on available balance)
  let SUPPLY_AMOUNT = USE_BASE_ASSET
    ? "1000000" // 1 USDC (6 decimals)
    : "1000000000000000"; // 0.001 WETH (18 decimals)

  console.log("Performing supply transaction to Compound Comet on Sepolia...");
  console.log(`  Comet market: ${COMET_ADDRESS}`);
  console.log(`  Asset: ${ASSET_LABEL} (${ASSET})`);
  console.log(`  Target Amount: ${ethers.formatUnits(SUPPLY_AMOUNT, ASSET_DECIMALS)} (will adjust based on balance)`);
  ...
  try {
    ...
    if (!isWETH) {
      ...
      if (currentAllowance >= BigInt(SUPPLY_AMOUNT)) {
        ...
        const approveTx = await tokenWithWallet.approve(COMET_ADDRESS, SUPPLY_AMOUNT);
    ...
    const supplyTx = await comet.supply(ASSET, SUPPLY_AMOUNT);
