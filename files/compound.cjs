  ...
  try {
    ...
    async function handleSupply() {
      ...
      if (balance < BigInt(AMOUNT)) {
        ...
        if (balance === 0n) {
          console.log(`  No ${ASSET_LABEL} balance — trying common testnet faucet functions...`);
          const faucetAbi = [
            "function mint(uint256 amount)",
            "function mint(address to, uint256 amount)",
            "function tap()",
          ];
          const faucetToken = new ethers.Contract(ASSET, faucetAbi, wallet);
          const mintAmount = ethers.parseUnits("1", ASSET_DECIMALS);
          let minted = false;
          for (const fn of ["tap", "mint(uint256)", "mint(address,uint256)"]) {
            try {
              const tx = fn === "tap"
                ? await faucetToken.tap()
                : fn === "mint(uint256)"
                ? await faucetToken["mint(uint256)"](mintAmount)
                : await faucetToken["mint(address,uint256)"](wallet.address, mintAmount);
              console.log(`  Faucet tx (${fn}): ${tx.hash}`);
              await tx.wait();
              minted = true;
              break;
            } catch (e) {
          ...
          if (minted) {
            ...
            console.log(`  ✅ New balance: ${ethers.formatUnits(balance, ASSET_DECIMALS)}`);
          }

          if (balance === 0n) {
            console.error(`❌ No ${ASSET_LABEL} balance, and no faucet function worked on this token.`);
            console.log("\nOptions:");
            console.log(`1. Get ${ASSET} from Compound's Sepolia faucet UI (app.compound.finance, testnet mode)`);
            console.log("2. Set USE_BASE_ASSET=true to supply USDC instead (classified as Repay, not Supply)");
            process.exit(1);
          }
        }
      }
